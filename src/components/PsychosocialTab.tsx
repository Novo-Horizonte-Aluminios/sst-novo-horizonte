  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-sm font-black tracking-tight text-slate-800 uppercase">NR-01: Avaliação de Risco Psicossocial</h3>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {assessments.map(assess => (
            <div key={assess.id} className="bg-white p-5 rounded-2xl border border-slate-200 hover:shadow-md transition shadow-sm flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <div>
                  <h4 className="font-black text-[14px] text-slate-800 leading-snug tracking-tight">{assess.employeeName}</h4>
                  <p className="text-slate-450 font-mono text-[9px] mt-0.5">Avaliador: {assess.evaluator}</p>
                </div>

                <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 p-3 rounded-xl border border-slate-100 text-slate-650 font-bold">
                  <div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-xl rounded-2xl shadow-xl overflow-hidden animate-fade-in text-xs border border-slate-100">
            ...
            <form onSubmit={handleCreateAssessment} className="p-6 space-y-4">
              ...
              <div>
                <label className="font-semibold block mb-1 text-slate-600">Avaliador Responsável</label>
                <input
                  type="text"
                  required
                  value={evaluator}
              ...
              <div className="space-y-3 pt-2 border-t border-slate-100">
                <p className="font-bold text-slate-700 mb-2">Responda as questões (1 = Discordo Totalmente, 5 = Concordo Totalmente)</p>
                {questions.map(q => (
                  <div key={q.key} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100">
                    <span className="font-semibold text-slate-700 text-[11px] sm:max-w-xs">{q.label}</span>
