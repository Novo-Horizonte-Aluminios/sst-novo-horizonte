...
export default function BackupTab() {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm font-sans divide-y divide-slate-100 flex-1 flex flex-col min-w-0" id="data-backup-module">
      {/* Tab Header & Title */}
      <div className="p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-3 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
        <div>
        ...
        {/* Tab Controls */}
        <div className="flex bg-slate-200 dark:bg-slate-700/70 p-0.5 rounded border border-slate-200 dark:border-slate-700 gap-1 shrink-0 text-[10.5px]">
          <button
      ...
      {/* Loading overlay */}
      {loading ? (
        <div className="p-12 text-center text-slate-500 dark:text-slate-400 flex flex-col items-center justify-center gap-2 animate-pulse">
          <RefreshCw className="w-7 h-7 text-safety-green animate-spin" />
          <span className="text-xs font-bold text-slate-700 dark:text-slate-200">Carregando dados de segurança de TI corporativos...</span>
        </div>
      ) : (
        <div className="p-4 flex-1 overflow-y-auto">
          ...
          {activeSubTab === 'status' && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-3.5 rounded flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start">
                      <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider font-mono">Upload Automatizado S3</span>
                    ...
                    <h3 className="text-base font-black text-slate-800 dark:text-slate-100 tracking-tight mt-1 shrink-0 truncate">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center text-[10px] text-slate-500 dark:text-slate-400 font-mono">
                ...
                <div className={`p-3.5 rounded border flex flex-col justify-between transition-colors ${
                  config.maskSensitiveData 
                    ? 'bg-emerald-50/50 border-emerald-200 text-slate-800 dark:text-slate-100' 
                    : 'bg-rose-50/30 border-rose-200 text-slate-850'
                }`}>
                  <div>
                    <div className="flex justify-between items-start">
                      <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider font-mono">Conformidade LGPD</span>
                      <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider border ${
                        config.maskSensitiveData 
                    ...
                    <h3 className="text-base font-black tracking-tight mt-1 flex items-center gap-1.5 text-slate-800 dark:text-slate-100">
                      {config.maskSensitiveData ? (
                        <>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-250 flex justify-between items-center text-[10px] text-slate-500 dark:text-slate-400 font-mono">
              ...
              <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded p-4">
                <h3 className="text-[11px] font-bold text-slate-750 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <Settings2 className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400 font-bold" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div className="bg-white dark:bg-slate-800 p-3.5 rounded border border-slate-200 dark:border-slate-700 flex flex-col justify-between">
                    <div>
                      <h4 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-1.5">
                        <FileCode className="w-4 h-4 text-indigo-500" />
                        DUMP Postgres SQL Local
                      </h4>
                      <p className="text-[10.5px] text-slate-500 dark:text-slate-400 leading-snug mt-1">
                  ...
                  <div className="bg-white dark:bg-slate-800 p-3.5 rounded border border-slate-200 dark:border-slate-700 flex flex-col justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveSubTab('config')}
                      className="mt-3.5 w-full text-center text-[9.5px] text-slate-650 font-bold hover:text-slate-800 dark:text-slate-100 transition py-1 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:bg-slate-700 border border-slate-205 rounded cursor-pointer"
                    >
          ...
          {activeSubTab === 'config' && (
            <form onSubmit={handleSaveConfig} className="space-y-4 max-w-2xl bg-slate-50 dark:bg-slate-900/50 p-4 rounded border border-slate-250">
              <div className="flex items-center gap-1.5 border-b border-slate-200 dark:border-slate-700 pb-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">Parâmetros do Armazenamento S3 Compatível</h3>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 sm:grid-cols-2 gap-3.5 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded px-2.5 py-1.5 focus:outline-none focus:border-safety-green text-[11px]"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded px-2.5 py-1.5 focus:outline-none focus:border-safety-green text-[11px] font-mono"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded px-2.5 py-1.5 focus:outline-none focus:border-safety-green text-[11px] font-mono leading-none"
                    />
              ...
              <div className="bg-white dark:bg-slate-800 p-3 border border-slate-200 dark:border-slate-700 rounded mt-4">
                <div className="flex items-start gap-2.5">
                  <input
                    ...
                    className="mt-1 cursor-pointer w-4 h-4 rounded text-safety-green border-slate-300 dark:border-slate-600 focus:ring-safety-green"
                  />
                  <div className="text-xs">
                    <label htmlFor="lgpd-mask-checkbox" className="font-extrabold text-slate-800 dark:text-slate-100 cursor-pointer block select-none uppercase text-[11px]">
                      Ativar Anonimização Preventiva de Colaboradores (LGPD & GDPR)
                    </label>
                    <p className="text-slate-500 dark:text-slate-400 mt-0.5 text-[10.5px] leading-relaxed">
              ...
              <div className="pt-3 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={fetchData}
                  className="px-3 py-1.5 border border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 rounded font-bold text-[11px] transition cursor-pointer"
                >
                  Cancelar
                </button>
                <button
          ...
          {activeSubTab === 'logs' && (
            <div className="space-y-3">
              <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-900 p-2 border border-slate-205 rounded">
                <div className="text-xs">
                  <span className="font-extrabold text-slate-750 uppercase text-[10.5px]">Histórico Geral de Backups de Segurança</span>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">As ações contendo exportações de dados geram registros imutáveis com tamanho de arquivo para prevenção de fraudes e auditoria interna de TI.</p>
                </div>
                <button
                  onClick={fetchData}
                  className="p-1 px-2.5 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded text-[10px] font-bold text-slate-700 dark:text-slate-200 flex items-center gap-1 shadow-sm transition shrink-0 cursor-pointer"
                >
              ...
              <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded overflow-hidden">
                <table className="w-full border-collapse text-left text-xs divide-y divide-slate-100">
                  <thead className="bg-slate-50 dark:bg-slate-900 text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-650">
                    {logs.length === 0 ? (
                      ...
                      logs.map((log: any) => (
                        <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900 transition">
                          <td className="p-3 font-mono font-bold text-slate-800 dark:text-slate-100 shrink-0">
                            {new Date(log.sentAt).toLocaleString('pt-BR')}
                          </td>
                          <td className="p-3 text-[11px] font-medium text-slate-700 dark:text-slate-200">
                            {log.executor}
                          </td>
                          <td className="p-3">
                            <span className="bg-slate-100 dark:bg-slate-800/80 px-1.5 py-0.5 rounded text-[9.5px] border border-slate-200 dark:border-slate-700 text-slate-650">
                              {log.type}
                            </span>
                          </td>
                          ...
                          <td className="p-3 font-mono text-[10px] text-slate-500 dark:text-slate-400 max-w-xs truncate" title={log.destination}>
                            {log.destination}
                          </td>
